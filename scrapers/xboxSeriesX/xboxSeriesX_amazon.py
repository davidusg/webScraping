...
for x in range(1, 3):
    r = requests.get(
        f'https://www.amazon.com.mx/s?k=xbox+series+x&page={x}&__mk_es_MX=ÅMÅŽÕÑ&qid=1628677622', headers=headers)
    ...
    for item in productList:
        ...
        if re.search(r'\bConsola Xbox Series X\b', name):
            shortName = name[:50]
            link = item.find(
            ...
            pd.DataFrame(xboxList).to_csv(
                "amazon-xbox-series-x.csv", index=False)
